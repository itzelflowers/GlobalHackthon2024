...
def app():
    st.title("Lugares por sectores")
    selected_option = st.radio("¿Qué sector quieres?", ('Comida', 'Cultura', 'Entretenimiento'))
    m = init_map()
    m = plot_map(m)
    lugares = db.child('Lugares').get().val()
    for l in lugares:
        lugar = db.child('Lugares').child(l).child('bss_type').get().val()
        if selected_option == lugar:
